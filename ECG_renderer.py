...
def draw_ECG_multilead_vanilla(ECG_info: tuple):
    ...
    pixels_to_m_v = 150 // 10  # 147 pixels /10mV
    ...
    ax.plot(x_axis_long_lead, ECG_info[1][0] * y_scaling_factor + pixels_to_m_v * 10, linewidth=0.5, color='black')
    b = ECG_info[0]
    cntr_trans = [0, 3, 6, 9, 1, 4, 7, 10, 2, 5, 8, 11]
    for extrnl_cntr in range(3):
        # Plot a calibration signal
        x_cal_signal = np.linspace(0, 0.2 * pixels_to_sec, num=np.floor(0.2 * pixels_to_sec))
        y_cal_signal = np.ones_like(x_cal_signal) * pixels_to_m_v * 10 + b[cntr_trans[extrnl_cntr * 4]][
            0] * y_scaling_factor + (4 - extrnl_cntr) / 4.5 * image_pixels_ratio[1] - pixels_to_m_v * 20
        y_cal_signal[-2:] = 0 + b[cntr_trans[extrnl_cntr * 4]][0] * y_scaling_factor + (4 - extrnl_cntr) / 4.5 * \
                            image_pixels_ratio[1] - pixels_to_m_v * 20
        ax.plot(x_cal_signal, y_cal_signal, linewidth=0.5, color='black')
        for cntr in range(4):
            ax.plot(np.linspace(cntr * image_pixels_ratio[0] // 4.7 + len(y_cal_signal),
                                cntr * image_pixels_ratio[0] // 4.7 + len(b[cntr_trans[cntr], :]) + len(y_cal_signal),
                                num=len(b[cntr_trans[cntr], :])),
                    b[cntr_trans[extrnl_cntr * 4 + cntr], :] * y_scaling_factor + (4 - extrnl_cntr) / 4.5 *
                    image_pixels_ratio[1] - pixels_to_m_v * 20,
    ...
    # plt.gcf().add_axes(ax)
    try:
        plt.savefig('test.png', dpi=300, quality=95, transparent=True, pad_inches=0.0, bbox_inches='tight',
...
def find_image_frames(Input_image):
    ...
    while not b[pivot_y, min_x]:  # Find min X
        min_x += 1
    while not b[pivot_y, max_x - 1]:  # Find min X
        max_x -= 1
    while not b[min_y, pivot_x]:  # Find min X
        min_y += 1
    while not b[max_y - 1, pivot_x]:  # Find min X
        max_y -= 1
