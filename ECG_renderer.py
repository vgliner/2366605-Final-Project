...
def draw_ECG_multilead_vanilla(ECG_info: tuple):
    ...
    for extrnl_cntr in range(3):
        ...
        y_cal_signal=np.ones_like(x_cal_signal)*pixels_to_mV*10+B[cntr_trans[extrnl_cntr*4]][0]*y_scaling_factor+(4-extrnl_cntr)/5*image_pixels_ratio[1]-extrnl_cntr/10*image_pixels_ratio[1]
        y_cal_signal[-2:]=0+B[cntr_trans[extrnl_cntr*4]][0]*y_scaling_factor+(4-extrnl_cntr)/5*image_pixels_ratio[1]-extrnl_cntr/10*image_pixels_ratio[1]
        ...
        for cntr in range(4):
            ...
            num=len(B[cntr_trans[cntr], :])),B[cntr_trans[extrnl_cntr*4+cntr], :]*y_scaling_factor+(4-extrnl_cntr)/5*image_pixels_ratio[1]-extrnl_cntr/10*image_pixels_ratio[1],
