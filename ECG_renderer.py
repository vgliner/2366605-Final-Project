...
def draw_ECG_multilead_vanilla(ECG_info: tuple):
    ...
    y_cal_signal=np.ones_like(x_cal_signal)*pixels_to_mV*10+ECG_info[1][0]*y_scaling_factor+pixels_to_mV*10
    y_cal_signal[-2:]=ECG_info[1][0]*y_scaling_factor+pixels_to_mV*10
    ax.plot(x_cal_signal,y_cal_signal,linewidth=1, color='black')        

    x_axis_long_lead=np.arange(len(ECG_info[1]))
    x_axis_long_lead+=len(x_cal_signal)
    ax.plot(x_axis_long_lead,ECG_info[1]*y_scaling_factor+pixels_to_mV*10,  linewidth=1, color='black')
    B=ECG_info[0]
